"""Write sorted relative VNNLIB paths from one folder into a CSV file."""
...
def save_vnnlib_paths_to_csv(input_dir, output_csv_path):
    """Save sorted relative VNNLIB file paths from a directory into a CSV file."""
    vnnlib_files = sorted(
        [
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Write sorted relative VNNLIB paths from one folder into a CSV file.",
    )
...
def main():
    """Run the relative-path VNNLIB CSV export."""
    args = parse_args()
