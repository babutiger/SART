# Canonical Standard MILP baseline entrypoint.
...
from sart.layerabs.layerabs_variants.standard_milp_variants import (
    get_variant_config,
)
...
def forward_propagation_standard_milp(
    weights,
    biases,
    input_size,
    hidden_sizes,
...
def run_standard_milp_verification(
    d,
    ...
    run_layerabs_verification(
        ...
        network_cls=network,
        forward_propagation_fn=forward_propagation_standard_milp,
        terminate_child_processes=terminate_all_child_processes,
        average_divisor=config.resolved_average_divisor(),
        reported_total=config.resolved_average_divisor(),
    )


def run_configured_standard_milp_variant(variant_name, d=None):
    config = get_variant_config(variant_name)
    if d is None:
        d = config.default_delta
    run_standard_milp_verification(d, variant_name=variant_name)


forward_propagation_puremilp_complete = forward_propagation_standard_milp
run_puremilp_complete_verification = run_standard_milp_verification
run_configured_puremilp_variant = run_configured_standard_milp_variant
