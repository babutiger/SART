...
def mnist_robustness_radius():
    net = network()
    net.load_nnet("../../models/mnist_new_18x200/mnist_net_new_18x200.nnet")
    property_list = ["../../mnist_properties/mnist_properties_18x200/mnist_property_" + str(i) + ".txt" for i in range(100)]


    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/mnist_new_18x200_deeppoly_radius_result_" + str(style_time) + ".txt", mode="w+", encoding="utf-8")
...
def test_robustness_number_poly(d):
    ...
    net = network()
    net.load_nnet("../../models/mnist_new_18x200/mnist_net_new_18x200.nnet")
    property_list = ["../../mnist_properties/mnist_properties_18x200/mnist_property_" + str(i) + ".txt" for i in
                     range(amount)]

    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open("../../result/original_result/mnist_new_18x200_deeppoly_number_result_delta_"+ str(d) +"_"+ str(style_time) +".txt", mode="w+", encoding="utf-8")
...
if __name__ == "__main__":
    ...
    test_robustness_number_poly(0.014)
