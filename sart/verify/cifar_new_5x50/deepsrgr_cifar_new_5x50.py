...
sys.stdout = Logger("../../result/log/" + script_name_without_extension + "_log_" + str(style_time) + ".txt",
                    sys.stdout)
...
class network(object):
    ...
    def find_robustness_number_srgrlp(self, PROPERTY, t, TRIM, WORKERS=28, SOLVER=cp.GUROBI):
...
def cifar_robustness_radius_deeppoly():
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/cifar_new_5x50_deeppoly_radius_result_" + str(style_time) + ".txt",
                mode="w+", encoding="utf-8")
...
def cifar_robustness_radius_lp():
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/cifar_new_5x50_deepsrgr_radius_result_" + str(style_time) + ".txt",
                mode="w+", encoding="utf-8")
...
def test_robustness_number_srgrlp(d):
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open("../../result/original_result/cifar_new_5x50_deepsrgr_number_result_delta_" + str(d) + "_" + str(
    ...
    for property_i in property_list:
        start_time = time.time()
        num_single = net.find_robustness_number_srgrlp(property_i, d, TRIM=True)
...
if __name__ == "__main__":
    # cifar_robustness_radius_deeppoly()

    # cifar_robustness_radius_lp()

    test_robustness_number_srgrlp(0.003)
