...
class network(object):
    ...
    def verify_lp_split(self, PROPERTY, DELTA, MAX_ITER=5, SPLIT_NUM=0, WORKERS=12, TRIM=False, SOLVER=cp.GUROBI,
            ...
            for splits_num in range(len(split_list)):
                ...
                for j in range(MAX_ITER):
                    ...
                    if (j == 0) and (verification_neuron.concrete_lowest_upper_global > 0):
                        unsafe_set_mpbp_algorithm.add(splits_num)
                    constraints = []
        ...
        print('Overall Unsafe Overapproximate(MPBP_algorithm)%:', len(unsafe_set_mpbp_algorithm) / len(split_list) * 100)
        if MODE == self.MODE_ROBUSTNESS:
            return True
        if MODE == self.MODE_QUANTITIVE:
            if verified_area < len(unsafe_set) / len(split_list):
                return [verified_area * 100, len(unsafe_set_mpbp_algorithm) / len(split_list) * 100]
            else:
                return [len(unsafe_set) / len(split_list) * 100, len(unsafe_set_mpbp_algorithm) / len(split_list) * 100]
    ...
    def find_robustness_number_mpbp(self, PROPERTY, t, TRIM=False):
...
def cifar_robustness_radius_mpbp():
    net = network()
    net.load_nnet("../../models/cifar_new_5x50/cifar_net_new_5x50.nnet")
    property_list = ["../../cifar_properties/cifar_properties_5x50/cifar_property_" + str(i) + ".txt" for i in
                     range(100)]

    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/cifar_new_5x50_mpbp_radius_result_" + str(style_time) + ".txt", mode="w+",
                encoding="utf-8")
...
def cifar_robustness_radius_lp():
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/cifar_new_5x50_deepmr_radius_result_" + str(style_time) + ".txt", mode="w+",
                encoding="utf-8")
...
def test_robustness_number_mpbp(d):
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open("../../result/original_result/cifar_new_5x50_mpbp_number_result_delta_"+ str(d) +"_"+ str(style_time) +".txt", mode="w+", encoding="utf-8")
    ...
    for property_i in property_list:
        start_time = time.time()
        num_single = net.find_robustness_number_mpbp(property_i, d, TRIM=True)
...
if __name__ == "__main__":
    ...
    # cifar_robustness_radius_mpbp()

    # cifar_robustness_radius_lp()

    test_robustness_number_mpbp(0.003)
