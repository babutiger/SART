...
def mnist_robustness_radius_lp():
    net = network()
    net.load_nnet("../../models/mnist_new_10x80/mnist_net_new_10x80.nnet")
    property_list = ["../../mnist_properties/mnist_properties_10x80/mnist_property_" + str(i) + ".txt" for i in range(7, 0, -1)]
    ...
    for property_i in property_list:
        ...
        delta_base = net.find_max_disturbance_lp(PROPERTY=property_i,L=int(delta_base*1000),R=int(delta_base*1000+63),TRIM=True,WORKERS=14,SOLVER=cp.CBC)
        end_time = time.time()
        property_i = property_i[46:]
        property_i = property_i[:-4]
        print(f"{property_i} -- delta_base : {delta_base}")
...
def acasxu_robustness_radius_lp():
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open("../../result/original_result/acasxu_deepsrgr_radius_result_lp.txt", mode="w+",
                encoding="utf-8")
