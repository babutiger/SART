...
class network(object):
                ...
                for j in range(MAX_ITER):
                    ...
                    for k in range(len(self.layers)):
                        ...
                        if cur_layer.layer_type == layer.INPUT_LAYER:
                            ...
                            for p in range(cur_layer.size):
                                constraints.append(
                                    variables[k][p] <= cur_neuron_list[p].algebra_upper_heurisitc[:-1] @ variables[
                                        k - 1] + cur_neuron_list[p].algebra_upper_heurisitc[-1])
    ...
    def deeppoly(self):
        ...
        def back_propagation(cur_neuron, i):
            ...
            cur_neuron.concrete_highest_lower_polylocal = max(cur_neuron.concrete_lower_heurisitc,
                                                              cur_neuron.concrete_lower_multi)
            cur_neuron.concrete_upper_multi = min(cur_neuron.concrete_upper)
            cur_neuron.concrete_lowest_upper_polylocal = min(cur_neuron.concrete_upper_heurisitc,
