...
class network(object):
    ...
    def verify_lp_split(self,PROPERTY,DELTA,MAX_ITER=5,SPLIT_NUM=0,WORKERS=12,TRIM=False,SOLVER=cp.GUROBI,MODE=0,USE_OPT_2=False):
        ...
        delta_list=[self.layers[0].neurons[i].concrete_upper_heuristic-self.layers[0].neurons[i].concrete_lower_heuristic for i in range(self.inputSize)]
        self.clear()
        self.deeppoly()
        verify_layer=self.layers[-1]
        verify_neuron_upper=np.array([neur.concrete_upper_heuristic for neur in verify_layer.neurons])
        ...
        for i in range(2**SPLIT_NUM):
            ...
            for j in range(SPLIT_NUM):
                if i & (2**j) == 0:
                    cur_split.append([self.layers[0].neurons[j].concrete_lower_heurisitc,(self.layers[0].neurons[j].concrete_upper_heurisitc+self.layers[0].neurons[j].concrete_lower_heurisitc)/2])
                else:
                    cur_split.append([(self.layers[0].neurons[j].concrete_upper_heurisitc+self.layers[0].neurons[j].concrete_lower_heurisitc)/2,self.layers[0].neurons[j].concrete_upper_heurisitc])
            for j in range(SPLIT_NUM,self.inputSize):
                cur_split.append([self.layers[0].neurons[j].concrete_lower_heurisitc,self.layers[0].neurons[j].concrete_upper_heurisitc])
        ...
        for i in verify_list:
            ...
            for splits_num in range(len(split_list)):
                ...
                for j in range(self.inputSize):
                    self.layers[0].neurons[j].concrete_lower_heurisitc=splits[j][0]
                    self.layers[0].neurons[j].concrete_algebra_lower_heurisitc=np.array([splits[j][0]])
                    self.layers[0].neurons[j].algebra_lower_heurisitc=np.array([splits[j][0]])
                    self.layers[0].neurons[j].concrete_upper_heurisitc=splits[j][1]
                    self.layers[0].neurons[j].concrete_algebra_upper_heurisitc=np.array([splits[j][1]])
                    self.layers[0].neurons[j].algebra_upper_heurisitc=np.array([splits[j][1]])
                self.clear()
                for j in range(MAX_ITER):
                    ...
                    for k in range(len(self.layers)):
                        ...
                        if cur_layer.layer_type==layer.INPUT_LAYER:
                            for p in range(cur_layer.size):
                                constraints.append(variables[k][p]>=cur_neuron_list[p].concrete_lower_heurisitc)
                                constraints.append(variables[k][p]<=cur_neuron_list[p].concrete_upper_heurisitc)
                            ...
                            assert(k>0)
                            for p in range(cur_layer.size):
                                constraints.append(variables[k][p]<=cur_neuron_list[p].algebra_upper_heurisitc[:-1]@variables[k-1]+cur_neuron_list[p].algebra_upper_heurisitc[-1])
                    ...
                    mppool.terminate()
                    for k in range(self.inputSize):
                        if resultlist[k*2]>=input_neurons[k].concrete_lower_heurisitc:
                            input_neurons[k].concrete_lower_heurisitc=resultlist[k*2]
                            input_neurons[k].concrete_algebra_lower_heurisitc=np.array([resultlist[k*2]])
                            input_neurons[k].algebra_lower_heurisitc=np.array([resultlist[k*2]])
                        #
                        if resultlist[k*2+1]<=input_neurons[k].concrete_upper_heurisitc:
                            input_neurons[k].concrete_upper_heurisitc=resultlist[k*2+1]
                            input_neurons[k].concrete_algebra_upper_heurisitc=np.array([resultlist[k*2+1]])
                            input_neurons[k].algebra_upper_heurisitc=np.array([resultlist[k*2+1]])
                ...
                if prob.status==cp.OPTIMAL:
                    area=1
                    for j in range(self.inputSize):
                        area*=(self.layers[0].neurons[j].concrete_upper_heurisitc-self.layers[0].neurons[j].concrete_lower_heurisitc)/delta_list[j]
    ...
    def deeppoly(self):
        ...
        def back_propagation(cur_neuron, i):
            ...
            cur_neuron.concrete_highest_lower_polylocal = max(cur_neuron.concrete_lower_heurisitc, cur_neuron.concrete_lower_multi)
            cur_neuron.concrete_upper_multi = min(cur_neuron.concrete_upper)
            cur_neuron.concrete_lowest_upper_polylocal = min(cur_neuron.concrete_upper_heurisitc, cur_neuron.concrete_upper_multi)
            ...
            if (cur_neuron.concrete_highest_lower_global==None) or (cur_neuron.concrete_highest_lower_global<cur_neuron.concrete_highest_lower_polylocal):
                cur_neuron.concrete_highest_lower_global = cur_neuron.concrete_highest_lower_polylocal
            if (cur_neuron.concrete_lowest_upper_global==None) or (cur_neuron.concrete_lowest_upper_global>cur_neuron.concrete_lowest_upper_polylocal):
...
def mnist_robustness_radius():
    ...
    file = open("../../result/original_result/mnist_new_10x80_nonlinerpoly_radius_result.txt", mode="w+", encoding="utf-8")
...
def mnist_robustness_radius_lp():
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open("../../result/original_result/mnist_new_10x80_nonlinerpoly_radius_result_lp.txt", mode="w+", encoding="utf-8")

    for property_i in property_list:
        start_time = time.time()
        delta_base = net.find_max_disturbance(PROPERTY=property_i, TRIM=True)
        print("DeepPoly Max Verified Distrubance:", delta_base)
        delta_base = net.find_max_disturbance_lp(PROPERTY=property_i,L=int(delta_base*1000),R=int(delta_base*1000+63),TRIM=True,WORKERS=12,SOLVER=cp.CBC)
        end_time = time.time()
        property_i = property_i[46:]
        property_i = property_i[:-4]
        print(f"{property_i} -- delta_base : {delta_base}")
