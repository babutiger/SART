...
from sart.utils.newfunction import softplus, gelu
            ...
            if cur_layer.layer_type == layer.AFFINE_LAYER:
                ...
                for j in range(cur_layer.size):
                    ...
                    # elif pre_neuron.concrete_lower + pre_neuron.concrete_upper <= 0:
                        ...
                        # print("alpha:", alpha)
                        temp_xi = 0.5*(abs(np.sum(pre_neuron.concrete_algebra_upper)).item() / np.sum(pre_neuron.concrete_algebra_upper))
                        cur_neuron.algebra_upper[j] = 0.5 + temp_xi
                        ...
                        # cur_neuron.algebra_upper[-1] = 0.5*abs(np.sum(pre_neuron.concrete_algebra_upper)).item() / np.sum(pre_neuron.concrete_algebra_upper)
                    ...
                    #     # cur_neuron.algebra_lower[j] = 0
                    #
                    #     # print("pre_neuron.concrete_algebra_lower:",pre_neuron.concrete_algebra_lower )
                    #     # print("pre_neuron.concrete_algebra_lower:",pre_neuron.concrete_algebra_lower.dtype)
                    #
                    #     cur_neuron.algebra_lower[j] = gelu(np.sum(pre_neuron.concrete_algebra_lower)).item() / np.sum(pre_neuron.concrete_algebra_lower)
                    #     cur_neuron.algebra_upper[j] = softplus(np.sum(pre_neuron.concrete_algebra_upper)).item() / np.sum(pre_neuron.concrete_algebra_upper)
                    #
                    #     # alpha = pre_neuron.concrete_upper / (pre_neuron.concrete_upper - pre_neuron.concrete_lower)
...
def mnist_robustness_radius():
    ...
    file = open("../../result/original_result/mnist_new_10x80_abs_radius_result.txt", mode="w+", encoding="utf-8")
...
def acas_robustness_radius():
    ...
    #     print("Recur:", recur)
        ...
        property_list = ["acas_properties/local_robustness_" + str(i) + ".txt" for i in range(1, 7)]
        for property_i in property_list:
            star_time = time.time()
...
def test_max_disturbance():
    net = network()
    net.load_nnet('../../models/mnist_new_10x80/mnist_net_new_10x80.nnet')
    net.load_robustness('../../mnist_properties/mnist_property_19.txt', 0.001, TRIM=True)
    net.deeppoly()
...
def test_robustness_number(d):
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open("../../result/original_result/mnist_new_10x80_nonlinerpoly_number_result_delta_"+ str(d) +".txt", mode="w+", encoding="utf-8")
